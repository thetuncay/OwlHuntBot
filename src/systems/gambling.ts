...
async function settleGamble(
  ...
  return withLock(playerId, 'gamble', async () => {
    return prisma.$transaction(async (tx) => {
      ...
      // Kazanilan coin'i liderboard sayacina ekle
      if (win && gain > 0) {
...
export async function slot(prisma: PrismaClient, playerId: string, bet: number): Promise<GambleResult> {
  return withLock(playerId, 'gamble', async () => {
    return prisma.$transaction(async (tx) => {
      ...
      const roll = weightedRandom(
        SLOT_TABLE.map((entry) => ({
          value: entry,
          weight: entry.chance,
        })),
      );
...
/**
 * Blackjack sonucunu dışarıdan alıp coin günceller.
  ...
  return withLock(playerId, 'gamble', async () => {
    return prisma.$transaction(async (tx) => {
      ...
      while (handValue(playerCards) < 17) {
        playerCards.push(drawCard(true));
      }
      while (handValue(dealerCards) < 17 || (handValue(dealerCards) === 17 && dealerCards.includes(11))) {
